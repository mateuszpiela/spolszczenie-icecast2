...
<body>
	...
	<xsl:for-each select="source">
		<xsl:choose>
			...
			<div class="roundbox">
				<div class="mounthead">
					<h3 class="mount">Punkt Montowania <xsl:value-of select="@mount" /></h3>
					<div class="right">
						<xsl:choose>
					...
					<table class="yellowkeys">
						<tbody>
							<xsl:if test="server_name">
								<tr>
									<td>Nazwa Strumienia:</td>
									<td><xsl:value-of select="server_name" /></td>
								</tr>
							</xsl:if>
							<xsl:if test="server_description">
								<tr>
									<td>Opis Strumienia:</td>
							...
							<xsl:if test="stream_start">
								<tr>
									<td>Rozpoczęto strumieniować od:</td>
			...
			<xsl:otherwise>
				<h3><xsl:value-of select="@mount" /> - Nie połączono</h3>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:for-each>
	<div id="footer">
